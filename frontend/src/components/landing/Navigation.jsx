  ...
  >
    ...
    >
      ...
      <a href="#" className="flex items-center gap-2 group">
        <span className={`font-display tracking-tight transition-all duration-500 font-bold ${isScrolled ? "text-xl text-primary" : "text-2xl text-primary"}`}>Hushh</span>
        <span className={`text-xs transition-all duration-500 ${isScrolled ? "text-muted-foreground" : "text-accent"}`}>{"\u26A1"}</span>
      </a>
      {
        /* Desktop Navigation */
      }
      <div className="hidden md:flex items-center gap-12">{navLinks.map((link) => <a
        key={link.name}
        href={link.href}
      ...
      <div className="hidden md:flex items-center gap-4">
        <button onClick={() => onOpenAuth('login')} className={`transition-all duration-500 ${isScrolled ? "text-xs text-foreground/70 hover:text-foreground" : "text-sm text-white/70 hover:text-white"}`}>Login</button>
        <Button
          size="sm"
          onClick={() => onOpenAuth('register')}
          className={`rounded-full transition-all duration-500 ${isScrolled ? "bg-primary hover:bg-primary/90 text-white px-4 h-8 text-xs" : "bg-accent hover:bg-accent/90 text-white px-6"}`}
        >Join Hushh</Button>
      </div>
      {
        /* Mobile Menu Button */
    ...
    ><div className="flex flex-col h-full px-8 pt-28 pb-8">
      ...
      <div className="flex-1 flex flex-col justify-center gap-8">{navLinks.map((link, i) => <a
        key={link.name}
        href={link.href}
